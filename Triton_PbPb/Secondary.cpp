...
void Secondary(const float cutDCAz = 1.f, const int cutTPCcls = 89, const double cutDCAxy = 0.10f, const double cutChi2TPC = 2.5f, const char *inFileDatName = "TreeOutData", const char *inFileMCName = "TreeOutMC", const char *inFileWDName = "EfficiencyHe3SecWd", const char *outFileName = "PrimaryHe3", const bool useWdInFit = false, const bool rebinLowStatisticsBin = true)
{
  ...
  TFile *inFileMCSec = TFile::Open(Form("%s/%s.root", kResDir, inFileMCName));
  ...
  for (int iMatt = 1; iMatt < 2; ++iMatt)
  {
    ...
    TH3F *fDCAdat = (TH3F *)inFileDat->Get(Form("%1.1f_%d_%1.2f_%1.2f/f%sDCAxyTOF", cutDCAz, cutTPCcls, cutDCAxy, cutChi2TPC, kAntimatterMatter[iMatt]));
    ...
    for (int iCent = 0; iCent < kNCentClasses; ++iCent)
    {
      ...
      //nUsedPtBins = 10;
      TH1D fPrimaryFrac(Form("f%sPrimFrac_%.0f_%.0f", kAntimatterMatter[iMatt], kCentBinsLimitsHe3[iCent][0], kCentBinsLimitsHe3[iCent][1]), Form("%.0f-%.0f%%", kCentBinsLimitsHe3[iCent][0], kCentBinsLimitsHe3[iCent][1]), kNPtBins, pTbins);
      TH1D fPrimaryRMS(Form("f%sPrimRMS_%.0f_%.0f", kAntimatterMatter[iMatt], kCentBinsLimitsHe3[iCent][0], kCentBinsLimitsHe3[iCent][1]), Form("%.0f-%.0f%%", kCentBinsLimitsHe3[iCent][0], kCentBinsLimitsHe3[iCent][1]), kNPtBins, pTbins);
      TH1D fSecondaryFrac(Form("f%sSecFrac_%.0f_%.0f", kAntimatterMatter[iMatt], kCentBinsLimitsHe3[iCent][0], kCentBinsLimitsHe3[iCent][1]), Form("%.0f-%.0f%%", kCentBinsLimitsHe3[iCent][0], kCentBinsLimitsHe3[iCent][1]), kNPtBins, pTbins);


      for (int iPtBin = 1; iPtBin < 5; ++iPtBin)
      { // loop on pT bins
        ...
        fDCAdatProj = fDCAdat->ProjectionZ(TString::Format("f%sDCAxyTOF_%.0f_%.0f_%.2f_%.2f", kAntimatterMatter[iMatt], fDCAdat->GetXaxis()->GetBinLowEdge(kCentBinsHe3[iCent][0]), fDCAdat->GetXaxis()->GetBinUpEdge(kCentBinsHe3[iCent][1]), fDCAdat->GetYaxis()->GetBinLowEdge(lowerPtBinIndex), fDCAdat->GetYaxis()->GetBinUpEdge(upperPtBinIndex)), kCentBinsHe3[iCent][0], kCentBinsHe3[iCent][1], lowerPtBinIndex, upperPtBinIndex);
        ...
        if ( (iMatt == 1) && /*( (minPt < 3.45f && iCent < 2) ||  */(minPt < 2.95f /* && iCent > 1 ) */ ))
        {
          ...
          canvNameTPC = TString::Format("f%sDCAxyTOF_%.0f_%.0f_%.2f_%.2f", kAntimatterMatter[iMatt], fDCAdat->GetXaxis()->GetBinLowEdge(kCentBinsHe3[iCent][0]), fDCAdat->GetXaxis()->GetBinUpEdge(kCentBinsHe3[iCent][1]), fDCAdat->GetYaxis()->GetBinLowEdge(lowerPtBinIndex), fDCAdat->GetYaxis()->GetBinUpEdge(upperPtBinIndex));
          ...
          // if (rebinLowStatisticsBin && iPtBin <2)
          // {
          ...
          if (useWdInFit)
          {
            fitter->Config().ParSettings(2).Set("wd", 0.);
          ...
          { // check on fit status
            ...
            leg.AddEntry(mc1, "primary ^{3}H");
            leg.AddEntry(mc2, "secondary ^{3}H");
            ...
            if (useWdInFit)
            {
              ...
              leg.AddEntry(mc3, "Secondary ^{3}H, weak decays");
              mc3->Draw("histosame");
            }
            mc2->Draw("histosame");
            mc1->Draw("histosame");
            ...
            TLatex frac1(-1, 0.75 * result->GetMaximum(), Form("#it{f}_{prim} = %.3f #pm %.3f", primaryRatio, primaryRatioError));
            frac1.SetTextSize(22);
            frac1.Draw("same");
            //canv.SetLogy();
            canv.Write();
      ...
      gStyle->SetStatY(0.5);
      TF1 fSigmoid(Form("f%sSigmoidFit_%.0f_%.0f", kAntimatterMatter[iMatt], kCentBinsLimitsHe3[iCent][0], kCentBinsLimitsHe3[iCent][1]), "([0])/(1+exp([1]*x+[2]))", 1.f, 3.f);
      fSigmoid.SetParName(0, "a");
      fSigmoid.SetParName(1, "b");
      fSigmoid.SetParName(2, "c");
      fSigmoid.FixParameter(0, 1.);
      if (iMatt == 1)
      {
      ...
      fPrimaryFrac.GetXaxis()->SetRangeUser(1., 3.);
      fPrimaryFrac.Write();
