...
for i_cent, cent in enumerate(centrality_classes):
    ...
    for sign in [-1.,1.,0.]:
        ...
        leg_ratios_particle = ROOT.TLegend(0.199248-0.01,0.601242-0.12,0.414787-0.018,0.785093-0.12)
        leg_ratios_particle.SetTextFont(43)
        leg_ratios_particle.SetTextSize(20)
        ...
        hNSigmaRatioFitParticle.SetLineColor(ROOT.kRed) #(centrality_colors[i_cent])
        hNSigmaRatioFitParticle.SetMarkerColor(ROOT.kRed) #(centrality_colors[i_cent])
        ...
        hRatiosParticle.GetYaxis().SetRangeUser(0.62,1.5)
        text_alice = ROOT.TLatex(0.18,0.82,"ALICE Preliminary")
        text_alice.SetNDC(True)
        text_alice.SetTextFont(63)
        text_alice.SetTextSize(20)
        text_energy = ROOT.TLatex(0.18,0.75,"Pb-Pb, #sqrt{#it{s}_{NN}}=5.02 TeV")
        text_energy.SetNDC(True)
        text_energy.SetTextFont(43)
        text_energy.SetTextSize(20)
        text_centrality = ROOT.TLatex(0.18,0.68,f"Centrality {cent[0]}-{cent[1]}%")
        text_centrality.SetNDC(True)
        text_centrality.SetTextFont(43)
        text_centrality.SetTextSize(20)
        gRatiosParticle = ROOT.TGraphErrors(hRatiosParticle)
        gRatiosParticleFit = ROOT.TGraphErrors(hRatiosParticleFit)
        for i_part in range(0,4):
            gRatiosParticle.SetPointError(i_part,0,hRatiosParticle.GetBinError(i_part+1))
...
text_alice = ROOT.TLatex(0.2,0.8,"ALICE Preliminary")
text_alice.SetTextFont(62)
text_alice.SetNDC()
text_energy = ROOT.TLatex(0.2,0.73,"Pb-Pb, #sqrt{#it{s}_{NN}} = 5.02 TeV")
text_energy.SetTextFont(42)
text_energy.SetNDC()
text_uncorr_uncertainty = ROOT.TLatex(0.2,0.66,"Uncorrelated uncertainties only")
text_uncorr_uncertainty.SetTextFont(42)
...
hMuBCentFrame.GetYaxis().SetRangeUser(0.,1.5)
...
#legMuBCent.AddEntry(gMuBCent,"Uncorrelated uncertainties only")
