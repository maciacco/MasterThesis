...
void pTShaping(const char *inFileMCName=kInFileMCName, const char *inFileCentName=kInFileCentName, const char *outFileName=kOutFileName){
  ...
  for (int iPart = 0; iPart < 3; ++iPart){
    ...
    outFile.cd(dir);
    TF1 *blastWave[nCent];
    std::string name[nCent];
    for (int iC = 0; iC < nCent; ++iC){
      double nEvInRange = hCent->Integral(cent[iC]+1,cent[iC+1]);
      name[iC] = "bw_";
      name[iC] += std::to_string(iC);
      blastWave[iC] = pwgFunc->GetBGBW(kMass[iPart],beta_max(beta_avg[iC],n[iC]),T_kin[iC],n[iC],1,name[iC].data());
      blastWave[iC] = pwgFunc->GetBGBW(kMass[iPart],beta_max(beta_avg[iC],n[iC]),T_kin[iC],n[iC],avg_mult[iC]*nEvInRange/nEv/blastWave[iC]->Integral(0,1.e3),name[iC].data());
    ...
    auto reweight = [hTmp, normMin, bw_max, hBW](float pT){
      ...
      bool cut = (gRandom->Rndm()*hNormVal*bw_max) < (normMin*0.99*bw);
      return cut;
    };
