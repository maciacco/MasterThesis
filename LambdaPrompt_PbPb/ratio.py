...
for i_cent_bins in range(len(CENTRALITY_LIST)):
    ...
    for ct_bins in zip(CT_BINS_CENT[i_cent_bins][:-1], CT_BINS_CENT[i_cent_bins][1:]):
        if ct_bins[0] < 5 or ct_bins[1] > 40:
            continue
        h_raw_a = raw_yields_file.Get(f"antimatter_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}_pol/fRawYields_pol")
        h_raw_m = raw_yields_file.Get(f"matter_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}_pol/fRawYields_pol")
        ...
        delta_t = ct_bins[1]-ct_bins[0]
        raw_yield_a = h_raw_a.GetBinContent(h_raw_a.FindBin(0.801))
        raw_yield_m = h_raw_m.GetBinContent(h_raw_m.FindBin(0.801))
        raw_yield_error_a = h_raw_a.GetBinError(h_raw_a.FindBin(0.801))
        raw_yield_error_m = h_raw_m.GetBinError(h_raw_m.FindBin(0.801))
        j = 1
        i = np.power(-1,j)*0.01*int(j/2)
        while (raw_yield_a < 1.e-6 or raw_yield_m < 1.e-6) and (0.801+i < 0.9) and (0.801+i > 0.7):
            i = np.power(-1,j)*0.01*int(j/2)
            j = j + 1
            raw_yield_a = h_raw_a.GetBinContent(h_raw_a.FindBin(0.801+i))
            raw_yield_m = h_raw_m.GetBinContent(h_raw_m.FindBin(0.801+i))
            raw_yield_error_a = h_raw_a.GetBinError(h_raw_a.FindBin(0.801+i))
            raw_yield_error_m = h_raw_m.GetBinError(h_raw_m.FindBin(0.801+i))
        ...
        h[0].SetBinError(h[0].FindBin(0.5*(ct_bins[0]+ct_bins[1])),raw_yield_error_a/delta_t/h_pres_eff_a.GetBinContent(h[0].FindBin(0.5*(ct_bins[0]+ct_bins[1]))))
        ...
        h[1].SetBinError(h[1].FindBin(0.5*(ct_bins[0]+ct_bins[1])),raw_yield_error_m/delta_t/h_pres_eff_m.GetBinContent(h[1].FindBin(0.5*(ct_bins[0]+ct_bins[1]))))
