...
if PRODUCE_DATASETS and not TRAIN:
    ...
    for i_cent_bins in range(len(CENTRALITY_LIST_TRAINING)):
        ...
        df_MC = ROOT.RDataFrame("LambdaTree",f"/data/mciacco/LambdaPrompt_PbPb/AnalysisResults_reweight_BW_{cent_bins[0]}_{cent_bins[1]}.root")
        df_index = df_MC.Define("index_1","gRandom->Rndm()+mass-mass")
        df_index.Filter("flag < 5").Define("y_true_from_flags","(flag==1)? 2 : 1").Snapshot("LambdaTree",f"/data/mciacco/LambdaPrompt_PbPb/trainingSample_reweight_BW_{cent_bins[0]}_{cent_bins[1]}.root")
        # df_index.Filter("index_1 < 0.5 && isReconstructed && flag < 5").Define("y_true_from_flags","(flag==1)? 2 : 1").Snapshot("LambdaTree","/data/mciacco/LambdaPrompt_PbPb/pseudodataSample_TEST_LOW_BACKGROUND.root")
        # df_index.Filter("index_1 < 0.5 && isReconstructed").Snapshot("LambdaTree","/data/mciacco/LambdaPrompt_PbPb/pseudodataSample_full_TEST_LOW_BACKGROUND.root")
        # df_index.Snapshot("LambdaTree","/data/mciacco/LambdaPrompt_PbPb/mc_TEST_LOW_BACKGROUND.root")
        # df_index.Filter("index_1 > 0.5 && isReconstructed").Snapshot("LambdaTree","/data/mciacco/LambdaPrompt_PbPb/trainingSample_TEST_LOW_BACKGROUND.root")#,{"pt","ct","mass","matter","isReconstructed","flag","isPrimary","eta","radius","centrality","hasITSrefit","cosPA", "dcaV0tracks", "dcaPiPV", "dcaPrPV", "dcaV0PV", "tpcNsigmaPr", "tpcNsigmaPi", "tpcClV0Pr", "tpcClV0Pi", "radius"})
        ...
        #df_index.Filter("index_1 > 0.05 && index_1 < 0.4 && (mass < 1.1 || mass > 1.135)").Define("y_true_from_flags","0+mass-mass").Snapshot("LambdaTree","/data/mciacco/LambdaPrompt_PbPb/pseudodataBackground_TEST_LOW_BACKGROUND.root")#,{"pt","ct","mass","matter","eta","radius","centrality","hasITSrefit","cosPA", "dcaV0tracks", "dcaPiPV", "dcaPrPV", "dcaV0PV", "tpcNsigmaPr", "tpcNsigmaPi", "tpcClV0Pr", "tpcClV0Pi", "radius"})
        #df_index.Filter(f"index_1 > 0.3 && centrality > {cent_bins[0]} && centrality < {cent_bins[1]}").Snapshot("LambdaTree",f"/data/mciacco/LambdaPrompt_PbPb/test_real_data_{cent_bins[0]}_{cent_bins[1]}.root")
        ...
        for ct_bins in CT_BINS:
            ...
            for split in SPLIT_LIST:
                ...
                if CREATE_TRAIN_TEST and (COMPUTE_SCORES_FROM_EFF or TRAIN):
                    ...
                    df_background_ct_ = df_background.query(f'ct > {ct_bins[0]} and ct < {ct_bins[1]} and pt > 0.5 and pt < 3.5 and tpcClV0Pi > 69 and tpcClV0Pr > 69 and radius > 3 and radius < 100 and dcaPrPV < 20 and dcaPiPV < 20 and dcaV0PV < 10 and eta < 0.8 and eta > -0.8')

                    n_prompt = df_prompt_ct.shape[0]
                    n_non_prompt = df_nonprompt_ct.shape[0]
                    n_background = df_background_ct_.shape[0]
                    df_background_ct = df_background_ct_.sample(frac=0.2*n_non_prompt/n_background)
                    ...
                    del df_prompt_ct, df_nonprompt_ct, df_background_ct, df_background_ct_
                    ...
                    train_test_data[0].to_parquet(f'df/train_data_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}.parquet.gzip',compression='gzip')
                    train_test_data[2].to_parquet(f'df/test_data_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}.parquet.gzip',compression='gzip')
                    # continue
                else:
                    train_test_data[0] = pd.read_parquet(f'df/train_data_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}.parquet.gzip')
                    train_test_data[2] = pd.read_parquet(f'df/test_data_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}.parquet.gzip')
                ...
                if use_gpu:
                    model_clf = xgb.XGBClassifier(use_label_encoder=False, n_jobs=40, tree_method="gpu_hist", max_bin=128, gpu_id=0)
                else:
                ...
                # write
                train_test_data_cent[0].to_parquet(f'df/train_data_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}_predict.parquet.gzip',compression='gzip')
                train_test_data_cent[2].to_parquet(f'df/test_data_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}_predict.parquet.gzip',compression='gzip')
...
if APPLICATION:
    ...
    for split in SPLIT_LIST:
        ...
        for i_cent_bins in range(len(CENTRALITY_LIST_TRAINING)):
            ...
            for ct_bins in CT_BINS:
                if (ct_bins[0] < 10 or ct_bins[1] > 15):
                    continue
                bin = f'{split}_mc_sig_{cent_bins[0]}_{cent_bins[1]}_{ct_bins[0]}_{ct_bins[1]}_0_5_reweight'
                ...
                if USE_PD:
                    ...
                    df_data = uproot.open(f'/data/mciacco/LambdaPrompt_PbPb/AnalysisResults_data.root')['LambdaTree'].arrays(library="pd")
                    #df_data = df_data.append(df_data_r, ignore_index=True)
                    df_data_cent = df_data.query(
                    f'isReconstructed and matter {split_ineq_sign} and centrality > {cent_bins[0]} and centrality < {cent_bins[1]} and pt > 0.5 and pt < 3.5 and ct > {ct_bins[0]} and ct < {ct_bins[1]} and tpcClV0Pi > 69 and tpcClV0Pr > 69 and radius > 3 and radius < 100 and dcaPrPV < 20 and dcaPiPV < 20 and dcaV0PV < 10 and eta < 0.8 and eta > -0.8')
                    del df_data
                    ...
                    df_data = TreeHandler()
                    df_data.get_handler_from_large_file(f"/data/mciacco/LambdaPrompt_PbPb/AnalysisResults_data.root", "LambdaTree",
