        ...
        for ct_ in CT_BINS:
            ...
            for ct_bins in zip(CT_BINS_CENT[i_cent_bins][:-1], CT_BINS_CENT[i_cent_bins][1:]):
                ...
                for bkg_function in ['bkg']:
                    ...
                    df_data_bkg = df_data.query(f"ct > {ct_bins[0]} and ct < {ct_bins[1]} and mass > 1.09 and mass < 1.15 and (mass < 1.105 or mass > 1.13) and matter {split_ineq_sign}")
                    ...
                    df_data_sig = df_mc_sig.query(f"ct > {ct_bins[0]} and ct < {ct_bins[1]} and mass > 1.09 and mass < 1.15 and index_1 < 0.3 and matter {split_ineq_sign}")
                    df_mc_sig = df_mc_sig.query(f"ct > {ct_bins[0]} and ct < {ct_bins[1]} and mass > 1.09 and mass < 1.15 and index_1 > 0.3")
                    ...
                    df_data_ = [df_data_bkg, df_data_sig]
                    df_data = pd.concat(df_data_)
                    ...
                    for bdt_score, bdt_eff in zip(score, eff_selected):
                        ...
                        inv_mass_roo_mc = helpers.ndarray2roo(mc_inv_mass_bkg.to_numpy(),inv_mass)
                        ...
                        bkg_pdf = ROOT.RooHistPdf('bkg','bkg',inv_mass,inv_mass_mc)
                        ...
                        h_p = bdt_mc_prompt_pdf.createHistogram("BDT out")
                        h_np = bdt_mc_non_prompt_pdf.createHistogram("BDT out")
                        h_b = bdt_mc_bkg_pdf.createHistogram("BDT out")

                        f = ROOT.TFile("out_1_mc_split.root","update")
                        f.cd()
                        ...
                        if bkg_function == 'pol':
                            cov_w_w = cov(10,9)
                            cov_n_w_signal = cov(7,9)
                            cov_n_w_np = cov(7,10)
                        elif bkg_function == 'expo':
                            cov_w_w = cov(9,8)
                            cov_n_w_signal = cov(6,8)
                            cov_n_w_np = cov(6,9)
