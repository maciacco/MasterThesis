...
for split in SPLIT_LIST:
    for i_cent_bins in range(len(CENTRALITY_LIST)):
        ...
        evts = np.sum(counts_cent_range)*0.25 # 25% of events used for bkg in data
        ...
        for ct_bins in zip(CT_BINS_CENT[i_cent_bins][:-1], CT_BINS_CENT[i_cent_bins][1:]):
            ...
            for eff_score in zip(eff_array, score_eff_arrays_dict[bin]):
                ...
                df_data_sel = df_data.query(f'model_output_2 > {eff_score[1]}')
                ...
                sig = expected_signal(cent_bins, ct_bins, eff, evts, cent_counts, cent_edges)[0]
                non_prompt = sig*0.1 # approx (SHM + BR(Xi->L) almost 100%)
                if not SPLIT:
                    sig *= 2
                ...
                significance_list.append(sig/np.sqrt(sig+bkg+non_prompt))
                significance_err_list.append(significance_error(sig, bkg+non_prompt))
