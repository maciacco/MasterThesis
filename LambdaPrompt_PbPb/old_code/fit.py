...
for i_cent_bins in range(len(CENTRALITY_LIST)):
    ...
    for ct_ in CT_BINS:
        ...
        for ct_bins in zip(CT_BINS_CENT[i_cent_bins][:-1], CT_BINS_CENT[i_cent_bins][1:]):
            ...
            df_data_ = df_data.query(f"mass > 1.09 and mass < 1.15 and pileUpCheck")
            df_data_sidebands = df_data_.query(f"mass < 1.095 or mass > 1.14").sample(frac=0.5)
            ...
            df_mc_ = df_mc_.query(f'ct > {ct_bins[0]} and ct < {ct_bins[1]} and mass > 1.09 and mass < 1.15 and (hasTOFhit or hasITSrefit)')
            ...
            for bdt_score, bdt_eff in zip(score, eff_selected):
                if bdt_eff < 0.79 or bdt_eff > 0.81: #or bdt_score > 0.15:
                    continue
                ...
                for i_split, split in enumerate(SPLIT_LIST):
                    ...
                    if SPLIT:
                        ...
                        if split == 'antimatter':
                            split_ineq_sign = '< 0.5'
                    df_mc_cut_bkg = df_data_sidebands.query(f"bdtOutputBackground < {bdt_score} and matter {split_ineq_sign}")
                    df_data_cut = df_data_.query(f"bdtOutputBackground < {bdt_score} and matter {split_ineq_sign}")
                    ...
                    mc_non_prompt_bdt_non_prompt_out = df_mc_cut.query("(flag == 2 or flag == 4) and (flag != 1 or flag != 0)")["model_output_prompt"]
