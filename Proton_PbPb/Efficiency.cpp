...
void Efficiency(const char *cutSettings = "", const char *inFileNameMC = "mc", const char *outFileNameEff = "EfficiencyProton_noFilterBit")
{
  // make signal extraction plots directory
  system(Form("mkdir %s/efficiency", kPlotDir));

  TFile inFile1(Form("%s/%s_finePtBinning1_noFilterBit.root", kDataDir, inFileNameMC));
  TFile inFile2(Form("%s/%s_finePtBinning2_noFilterBit.root", kDataDir, inFileNameMC));
  TFile inFile3(Form("%s/%s_finePtBinning3_noFilterBit.root", kDataDir, inFileNameMC));
