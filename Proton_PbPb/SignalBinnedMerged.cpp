  ...
  { // loop on antimatter/matter
    ...
    { // loop over centrality classes
      ...
      { // loop on pT bins
        ...
        if (ptMin < 2.01) extendNsigmaLeft = 4.;
        ...
        /* TF1 gaus("gausCore", "gaus(0)+gaus(3)", kTOFnSigmaMin, kTOFnSigmaMax);
        gaus.SetParLimits(0, 10., 1.e8);
        gaus.SetParLimits(1, 0., 2.0);
        gaus.FixParameter(1, maxProton);
        gaus.SetParLimits(2, 0.5, 1.5);
        gaus.SetParLimits(3, 1., 1.e9);
        gaus.SetParLimits(4, -9.0, -2.0);
        if (ptMin > 3.5)gaus.SetParLimits(4, -5.5, -3.0);
        gaus.FixParameter(4, maxMesons);
        gaus.SetParLimits(5, 0.5, 3.0);
        tofSignalProjection->Fit("gausCore", "QMRL", "", -6., 6.); */
        ...
        { // expo
          if (ptMin < 2.61)
          {
            ...
            if (iCent == 0 || iCent == 1)
            slope2 = new RooRealVar("#tau_{mesons}", "slope2", -10., -1.5);
            else
            slope2 = new RooRealVar("#tau_{mesons}", "slope2", -5., -1.5);
          ...
          {
            slope1 = new RooRealVar("#tau_{mismatch}", "slope1", -0.5, -0.01);
            mean2 = new RooRealVar("#mu_{2}", "mu2", -10., -2.0);
            sigma2 = new RooRealVar("#sigma_{2}", "sigma2", 0.5, 2.0);
            background1 = (RooAbsPdf *)new RooGausExp("background1", "background1", tofSignal, *mean2, *sigma2, *alpha2R);
        ...
        RooRealVar nSignal("N_{sig}", "nSignal", 1., 1.e9);
        RooRealVar nBackground("N_{bkg}", "nBackground", 1., 1.e7);
        ...
        if (extractSignal)
        {
          // fit model
          RooFitResult *r;
          for (int i = 0; i < 2; ++i) // TODO: repeat fit twice to obtain better convergence
          {
          ...
          fAlpha.SetBinContent(fAlpha.FindBin((ptMax + ptMin) / 2.), alpha1->getVal());
          fAlpha.SetBinError(fAlpha.FindBin((ptMax + ptMin) / 2.), alpha1->getError());
        }
