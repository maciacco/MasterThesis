...
void SystematicsPtNotCombinedCheck(const int points = kNPoints, const bool cutVar = true, const bool binCountingVar = true, const bool expVar = true, const bool sigmoidVar = true, const char *outFileName = "SystematicsAllEPtNotCombined")
{
  ...
  for (int iC = 0; iC < kNCentClasses; ++iC) // TODO: extend the analysis to the third centrality class as well
  {
      ...
      for (int iROI=0; iROI<3; ++iROI){
        for (int iG3G4Prim=0; iG3G4Prim<1; ++iG3G4Prim){ // 0 = use G3 (DO NOT USE G4 in this case)
          for (int iSigmoid=0; iSigmoid<1; ++iSigmoid){ // 0 = directly from TFF -> do not use fit function
            ...
            for(int iPtBins=1;iPtBins<nUsedPtBins;++iPtBins){
    ...
    for(int iPtBin=1;iPtBin<nUsedPtBins;++iPtBin){
    ...
    // * * * * * * * * * * * * * * * * * * * * * * * * * //
    TH2D fSystematicsCorrelationsDCAxy(Form("fSystematicsCorrelationsDCAxy_%.0f_%.0f", kCentBinsLimitsProton[iC][0], kCentBinsLimitsProton[iC][1]),Form("%.0f-%.0f%%", kCentBinsLimitsProton[iC][0], kCentBinsLimitsProton[iC][1]),5,0.95,1.45,1000,-.5,.5);
    ...
    hRatio.GetXaxis()->SetRangeUser(0.8,3.);
    ...
    //gRatio.Draw("P5 same");
    gRatio.GetXaxis()->SetRangeUser(0.8,3.);
    ...
    for (int iPtBin=1;iPtBin<nUsedPtBins;++iPtBin){  
      fSystematicUncertaintyTotalPtCorrelated.SetBinContent(iPtBin,fSystematicUncertaintyTotalPtCorrelated.GetBinContent(iPtBin)*fRatioFromVariationsTot.GetBinContent(iPtBin));
      double sigma_Eff=fSystematicUncertaintyEff.GetBinContent(iPtBin);
      double sigma_TFF=fSystematicUncertaintyTFF.GetBinContent(iPtBin);
      hRatio.SetBinError(iPtBin,hRatio.GetBinContent(iPtBin)*TMath::Sqrt(sigma_Eff*sigma_Eff+sigma_TFF*sigma_TFF));
    }
    hRatio.Draw("pesame");
    ...
    p0.Draw("same");
    cRatio.Modified();
    cRatio.Print(Form("%s/%sCheck.pdf", kPlotDir, hRatio.GetName()));
    ...
    fSystematicUncertaintyTotal.GetXaxis()->SetRangeUser(0.8,3.);
    ...
    cSysError.Modified();
    cSysError.Print(Form("plots/%sCheck.pdf",fSystematicUncertaintyTotal.GetName()));
    cSysError.Write();
