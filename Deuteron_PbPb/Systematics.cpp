...
void Systematics(const int points = kNPoints, const bool cutVar = true, const bool binCountingVar = true, const bool expVar = true, const bool sigmoidVar = true, const char *outFileName = "SystematicsAll")
{
  ...
  TFile *specFile = TFile::Open(Form("%s/SpectraDeuteronSyst.root", kOutDir));
  ...
  for (int iC = 0; iC < kNCentClasses; ++iC)
  {
    ...
    TH1D fFitPar(Form("fFitPar_%.0f_%.0f", kCentBinsLimitsDeuteron[iC][0], kCentBinsLimitsDeuteron[iC][1]), Form("%.0f-%.0f%%", kCentBinsLimitsDeuteron[iC][0], kCentBinsLimitsDeuteron[iC][1]), 2000, 0.8, 1.0);
    ...
    while (iP < kNPoints)
    {
      for (int iPtBin = 3; iPtBin < nUsedPtBins + 3; ++iPtBin)
      {
        ...
        TH1D *h = (TH1D *)specFile->Get(Form("%s/fRatio_%.0f_%.0f", hname, kCentBinsLimitsDeuteron[iC][0], kCentBinsLimitsDeuteron[iC][1]));
