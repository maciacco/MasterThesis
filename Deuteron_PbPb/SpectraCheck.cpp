...
void SpectraCheck(const char *cutSettings = "", const bool binCounting = false, const int bkg_shape = 1, const bool sigmoidCorrection = true, const char *histoNameDir = ".", const char *outFileName = "SpectraDeuteronNoPrimary", const char *outFileOption = "recreate", const char *dataFile = "AnalysisResults", const char *signalFile = "SignalDeuteron", const char *effFile = "EfficiencyDeuteron", const char *primFile = "PrimaryDeuteron")
{
  gStyle->SetOptFit(0);
  ...
  for (int iCent = 0; iCent < kNCentClasses; ++iCent)
  {
    ...
    for (int iMatt = 0; iMatt < 2; ++iMatt)
    {
      ...
      for (int iPtBin = 2; iPtBin < pTbinMax + 1; ++iPtBin)
      {
        ...
        fSpectra[iMatt]->SetBinContent(iPtBin, rawYield / efficiency );
        fSpectra[iMatt]->SetBinError(iPtBin, (rawYield / efficiency) * TMath::Sqrt(effError * effError / efficiency / efficiency + rawYieldError * rawYieldError / rawYield / rawYield));
    ...
    fRatio[iCent]->GetXaxis()->SetRangeUser(0.,5.);
