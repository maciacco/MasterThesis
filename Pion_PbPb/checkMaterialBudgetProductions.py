...
input_eff_r_material = ROOT.TFile.Open("out/EfficiencyPionMC_21l5_false__LHC22b9.root")
input_eff_std_material = ROOT.TFile.Open("out/EfficiencyPionMC_21l5_false__LHC22b9_2.root")

cent = [[0,90],[999,999]]
matt = [["A","piMinus"],["M","piPlus"]]
outlier = [1.14,1.16,1.21,1.26,1.31,1.36]
...
for cent_class in cent:
    if cent_class[0]<100:
        for matt_label in matt:
            ...
            l = ROOT.TLegend(0.4,0.6,0.85,0.85)
            h_std = input_eff_std_material.Get(f"_/f{matt_label[0]}Eff_TOF_{cent_class[0]}_{cent_class[1]}")
            h_r = input_eff_r_material.Get(f"_/f{matt_label[0]}Eff_TOF_{cent_class[0]}_{cent_class[1]}")
            # if cent_class[0]>-1:
            #     for bin in outlier:
            ...
            h_std.GetXaxis().SetRangeUser(0.,3.)
            h_r.GetXaxis().SetRangeUser(0.,3.)
            ...
            h_rr.Fit("pol0","R","",0.5,1.6)
            lH = ROOT.TLine(0.25,1.,1.8,1.)
