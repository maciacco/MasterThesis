...
for i_cent_bins in range(len(CENTRALITY_LIST)):
    ...
    for ct_bins in zip(CT_BINS_CENT[i_cent_bins][:-1], CT_BINS_CENT[i_cent_bins][1:]):
        ...
        delta_t = ct_bins[1]-ct_bins[0]
        raw_yield_a = h_raw_a.GetBinContent(h_raw_a.FindBin(0.85))
        raw_yield_m = h_raw_m.GetBinContent(h_raw_m.FindBin(0.85))
        raw_yield_error_a = h_raw_a.GetBinError(h_raw_a.FindBin(0.85))
        raw_yield_error_m = h_raw_m.GetBinError(h_raw_m.FindBin(0.85))
        j = 1
        i = np.power(-1,j)*0.01*int(j/2)
        while (raw_yield_a < 1.e-6 or raw_yield_m < 1.e-6) and (0.85+i < 0.91) and (0.85+i > 0.75):
            i = np.power(-1,j)*0.01*int(j/2)
            j = j + 1
            raw_yield_a = h_raw_a.GetBinContent(h_raw_a.FindBin(0.85+i))
            raw_yield_m = h_raw_m.GetBinContent(h_raw_m.FindBin(0.85+i))
            raw_yield_error_a = h_raw_a.GetBinError(h_raw_a.FindBin(0.85+i))
            raw_yield_error_m = h_raw_m.GetBinError(h_raw_m.FindBin(0.85+i))
    ...
    # systematics
    # h_sys = ROOT.TH1D(f"h_sys",f"h_sys",100,0.855,1)
    # i=0
    # while i < N_TRIALS:
    #     h_tmp = []
