...
for split in SPLIT_LIST:
    ...
    for i_cent_bins in range(len(CENTRALITY_LIST)):
        ...
        for ct_bins in zip(CT_BINS_CENT[i_cent_bins][:-1], CT_BINS_CENT[i_cent_bins][1:]):
            if (ct_bins[0] < 0 or ct_bins[1] > 10): # or (ct_bins[0] > 32 and ct_bins[1] < 35):
                continue
            ...
            raw_yield = h_raw.GetBinContent(h_raw.FindBin(0.85))/0.85
            raw_yield_error = h_raw.GetBinError(h_raw.FindBin(0.85))/0.85
            j = 0
            i = np.power(-1,j)*0.01*int(j/2)
            while raw_yield < 1.e-6 and (0.85+i < 0.91) and (0.85+i > 0.7):
                i = np.power(-1,j)*0.01*int(j/2)
                j = j + 1
                raw_yield = h_raw.GetBinContent(h_raw.FindBin(0.85+i))/(0.85+i)
                raw_yield_error = h_raw.GetBinError(h_raw.FindBin(0.85+i))/(0.85+i)
            ...
            h.SetBinError(h.FindBin(0.5*(ct_bins[0]+ct_bins[1])),raw_yield_error/delta_t/h_pres_eff.GetBinContent(h.FindBin(0.5*(ct_bins[0]+ct_bins[1])))) #raw_yield*h_pres_eff.GetBinError(h.FindBin(0.5*(ct_bins[0]+ct_bins[1])))/delta_t/h_pres_eff.GetBinContent(h.FindBin(0.5*(ct_bins[0]+ct_bins[1])))/h_pres_eff.GetBinContent(h.FindBin(0.5*(ct_bins[0]+ct_bins[1]))))
        h.Fit("expo","IMR+","",0,40)
        h.GetXaxis().SetTitle("#it{c}t (cm)")
        h.GetYaxis().SetTitle("d#it{N}/d(#it{c}t) (cm^{-1})")
        lifetime = ROOT.TLatex(1,5000,"#it{c}#tau = " + str(-1./h.GetFunction("expo").GetParameter(int(1))/speed_of_light) + " +/- "  + str(h.GetFunction("expo").GetParError(1)/h.GetFunction("expo").GetParameter(1)/h.GetFunction("expo").GetParameter(1)/speed_of_light) + " ps")
        f.cd()
        ...
        while i < N_TRIALS:
            ...
            for ct_bins in zip(CT_BINS_CENT[i_cent_bins][:-1], CT_BINS_CENT[i_cent_bins][1:]):
                if ct_bins[0] < 0 or ct_bins[1] > 40:
                    continue
                cut_rndm = ROOT.gRandom.Rndm()*0.15+0.75
                ...
                h_tmp.SetBinError(h_tmp.FindBin(0.5*(ct_bins[0]+ct_bins[1])),h_raw.GetBinError(h_raw.FindBin(cut_rndm+0.001))/delta_t/h_raw.GetXaxis().GetBinLowEdge(h_raw.FindBin(cut_rndm+0.001))/h_pres_eff.GetBinContent(h_tmp.FindBin(0.5*(ct_bins[0]+ct_bins[1]))))
            
            if h_tmp.GetEntries() < 0.5:
                continue
            h_tmp.Fit("expo")
            i = i+1
            if h_tmp.GetFunction("expo").GetNDF()< 4:
                continue
            if h_tmp.GetFunction("expo").GetChisquare()/h_tmp.GetFunction("expo").GetNDF()>3:
                continue
            #h_tmp.Write()
