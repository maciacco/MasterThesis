...
for i_cent, cent in enumerate(centrality_classes):
    ...
    for sign in ["Minus","Plus","STD"]:
        ...
        fit_expo = ROOT.TF2(f"fit_expo_{cent[0]}_{cent[1]}", "TMath::Exp(-2./3.*[0]*x-2.*[0]*[1]*y)", -0.5, 9.5,-1.05,2.05,"")
        ...
        print(f"mu_B (T = 156.2 MeV) = {fit_parameter_0*156.2} +/- {fit_parameter_error_0*156.2} +/- {fit_parameter_0*1.5} MeV; mu_Q (T = 156.2 MeV) = {fit_parameter_1} +/- {fit_parameter_error_1}")
        ...
        formatted_mu_I = "{:.2f}".format(fit_parameter_1)
        mu_I_error = np.sqrt(fit_parameter_error_1*fit_parameter_error_1/fit_parameter_1/fit_parameter_1)*np.abs(fit_parameter_1)
        formatted_mu_I_error = "{:.2f}".format(np.abs(fit_parameter_error_1))
        ...
        if sign!="STD":
            ...
            results_muI.append(fit_parameter_1)
            continue
        ...
        gMuICent.AddPoint(n_part[i_cent],fit_parameter_1)
        gMuICent.SetPointError(gMuBCent.GetN()-1,0.,mu_I_error)
        gMuICentCorrUnc.AddPoint(n_part[i_cent],fit_parameter_1)
