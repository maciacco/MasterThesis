          ...
          for (int iBin = 0; iBin < 2; ++iBin)
          {
            for (int iBkg = 0; iBkg < 2; ++iBkg)
            {
              ...
              for (int iSgm = 0; iSgm < 2; ++iSgm)
              {
                ...
                if (kCutTPCClusters[iCls] < 100)
                  snprintf(hname2, 24, "%1.1f_0%d_%1.2f_%1.2f_%d_%d_%d", kCutDCAz[iDCA], kCutTPCClusters[iCls], kCutDCAxy[iDCAxy], kCutChi2TPC[iChi2TPC], binCountingFlag, expFlag, sigmoidFlag);
                else
                  snprintf(hname2, 24, "%1.1f_%d_%1.2f_%1.2f_%d_%d_%d", kCutDCAz[iDCA], kCutTPCClusters[iCls], kCutDCAxy[iDCAxy], kCutChi2TPC[iChi2TPC], binCountingFlag, expFlag, sigmoidFlag);
